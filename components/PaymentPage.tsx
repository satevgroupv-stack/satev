...
interface Props {
  id?: string;
    ...
    let toastOutStockid: string | undefined = undefined;
...
const totalSum = useMemo(() => {
  ...
  const onPay = async () => {
    ...
    try {
      ...
      if (checkout_url) {
        ...
        toast.dismiss(forwardToastId);
      }
      // Optionally, handle response data here
    } catch (error: any) {
      toast.dismiss(toastId);
      toast.dismiss(toastOutStockid ?? "");
      id = toast.error(error?.response?.data?.error || "Payment failed, please try again.");
        setTimeout(() => {
          toast.dismiss(toastOutStockid ?? "");
        } , 5000);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] p-6 text-white ">
      ...
      <div className="bg-[#ff7101] py-3 text-center font-medium flex items-center justify-center rounded-xl mb-6">
        Order reserved! Please select payment method.
        <button
          type="button"
          onClick={() => window.history.back()}
          className="underline text-white ml-4 cursor-pointer"
        >
          See Orders
        </button>
      </div>
